...
void KPlotAxis::setTickMarks( double x0, double length ) {
	...
	double t = length / s;
        ...
        qDebug() << "label:" << this->label() << "length:" << length
                << "\tpwr: " << pwr
                << "\ts:" << s
                << "\tt" << t;
	...
	//position beyond the "minimum" edge of the data range.
        double Tick0 = x0 - fmod( x0, TickDistance/100 );
qDebug() << __LINE__ << "x0:" << x0 << "\tTick0:" << Tick0
        << "\tTickDistance:" << TickDistance << "fmod:" << fmod( x0, TickDistance )
        << "numMajorTickMarks" << NumMajorTicks;
...
QString KPlotAxis::tickLabel( double val ) const {
	if ( d->m_labelFmt == 't' ) {
		...
		int h = int(val);
                int m = 60.*(val - h);
                kDebug() << QString("val: ") + QString::number(val)
